...
func (ic *ContainerEngine) ContainerUpdate(ctx context.Context, updateOptions *entities.ContainerUpdateOptions) (string, error) {
	...
	var containerOptions getContainersOptions
	if updateOptions.Latest {
		containerOptions = getContainersOptions{latest: true}
	} else {
		containerOptions = getContainersOptions{names: []string{updateOptions.NameOrID}}
	}
	
	containers, err := getContainers(ic.Libpod, containerOptions)
	if err != nil {
		return "", err
	}
	if len(containers) != 1 {
